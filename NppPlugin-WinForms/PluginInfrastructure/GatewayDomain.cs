...
public class TextRange : IDisposable
{
    ...
    public nint NativePointer { get { InitNativeStruct(); return _ptrSciTextRange; } }

    public string Text { get { ReadNativeStruct(); return Marshal.PtrToStringAnsi(_sciTextRange.lpstrText)!; } }

    public CharacterRange Range { get { ReadNativeStruct(); return _sciTextRange.chrg; } set { _sciTextRange.chrg = value; InitNativeStruct(); } }

    private void InitNativeStruct()
    {
        if (_ptrSciTextRange == nint.Zero)
            _ptrSciTextRange = Marshal.AllocHGlobal(Marshal.SizeOf<Sci_TextRange>());
        Marshal.StructureToPtr(_sciTextRange, _ptrSciTextRange, false);
    }

    private void ReadNativeStruct()
    {
        if (_ptrSciTextRange != nint.Zero)

...
namespace ___NppPlugin___.PluginInfrastructure;
...
[StructLayout(LayoutKind.Sequential)]
public struct ShortcutKey(bool isCtrl, bool isAlt, bool isShift, Keys key)
{
    public byte _isCtrl = Convert.ToByte(isCtrl);
    public byte _isAlt = Convert.ToByte(isAlt);
    public byte _isShift = Convert.ToByte(isShift);
    public byte _key = Convert.ToByte(key);
}
...
public partial class FuncItems : IDisposable
{
    ...
    public void Add(FuncItem funcItem)
    {
        ...
        if (funcItem._pShKey._key != 0)
        {
            ...
            Marshal.WriteIntPtr(ptrPosNewItem, nint.Zero);
    ...
    public void Dispose()
    {
        if (!_disposed)
        {
            foreach (nint ptr in _shortCutKeys)
...
[Flags]
[System.Diagnostics.CodeAnalysis.SuppressMessage("Design", "CA1069:Enumerationswerte dürfen nicht dupliziert werden", Justification = "<Ausstehend>")]
